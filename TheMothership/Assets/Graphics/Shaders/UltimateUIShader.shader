...
Shader "SumFX/Ultimate Unlit-UI" {
    Properties {
        ...
        _MainMaskRotation ("Main Mask Rotation", Float ) = 0
        _MainMaskXMov ("Main Mask X Mov", Float ) = 0
        _MainMaskYMov ("Main Mask Y Mov", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _MainMaskRotation;
            uniform float _MainMaskYMov;
            uniform float _MainMaskXMov;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 uuuf3434 = _Time + _TimeEditor;
                float g_ang = uuuf3434.g;
                ...
                float c_ang = uuuf3434.g;
                float c_spd = _MainMaskRotation;
                float c_cos = cos(c_spd*c_ang);
                float c_sin = sin(c_spd*c_ang);
                float2 c_piv = float2(0.5,0.5);
                float2 c = (mul(i.uv0-c_piv,float2x2( c_cos, -c_sin, c_sin, c_cos))+c_piv);
                float4 a = _Time + _TimeEditor;
                float4 b = _Time + _TimeEditor;
                float2 d = ((c+float2(0.0,(_MainMaskYMov*a.g)))+float2((_MainMaskXMov*b.g),0.0));
                float4 _MainMask_var = tex2D(_MainMask,TRANSFORM_TEX(d, _MainMask));
                return fixed4(finalColor,_MainMask_var.a);
            }
            ENDCG
        }
    }
