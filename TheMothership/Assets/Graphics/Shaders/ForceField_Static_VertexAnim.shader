...
Shader "FORGE3D/Force Field/Force Field Static Vertex Animation Edge Falloff"
{
	Properties
	{			
		...
		Category
		{
			...
			SubShader
			{
				Pass
				{
					...
					fixed _VertexTile, _VertexSpeed, _VertexPower, _VertexFalloff;
				...
				v2f vert (a2v v)
				{
					...
					v.vertex.xyz += v.normal * _Offset + vMod * rDot;
				...
				{	
					...
					fixed fTex =  tex2D(_FieldTex, uvPanner(i.uv, _FieldPanSpeed, _FieldPanSpeed));
					...
					fixed3 st = fieldColor *  fTex * _Static;
					...
					return fixed4((st + final) * viewDot , 1.0);
				//	return fixed4(viewDot.xxxx);
				}
